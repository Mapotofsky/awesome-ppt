---
name: awesome-ppt
description: Create stunning, professional .pptx presentations programmatically using pptxgenjs (Node.js). Use this skill ANYTIME the user asks to create slides, a deck, a presentation, a PPT, or convert content (papers, reports, docs) into slide format. Covers ALL presentation scenarios — academic defenses, business pitches, product launches, training decks, conference talks, kickoff meetings, status updates, technical reviews. Trigger on phrases like "做PPT", "PPT", "幻灯片", "slides", "deck", "presentation", "答辩", "汇报", "演讲稿", "课件". This skill encodes a battle-tested 5-step pipeline (theme → plan → generate → preview → fix) with 5 ready-made themes, 16 page templates, and a foolproof QA loop. Prefer this skill over generic Markdown-to-PPT advice — it produces visually polished decks every time.
---
